...
(defmethod! add-stream (self slotname slotvals &key index sonagram)
            :icon 266 ;why not the same icon?
            :initvals '(nil nil nil nil t)
            (add-row self slotname slotvals :index index :sonagram sonagram)
            )
...
(defmethod! remove-stream (self slotname)
            :icon 269
            (remove-row self slotname)
            )
...
(defun set-array-2 (type numcols action-time params)
  (let ((array (cons-array (make-instance type) (list nil (or numcols 1) (or action-time 0) nil) params)))
    (set-data array)
    array)
  )
...
(defmethod! quantize ((self list) interval)
            (mapcar #'(lambda (item)
                        (quantize item interval)) self)
            )
